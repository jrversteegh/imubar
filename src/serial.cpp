...
static void uart_cb(const struct device *dev, void *data) {
  ...
  if (uart_irq_tx_ready(dev) > 0) {
    uint8_t *write_buf;
    auto to_write =
        ring_buf_get_claim(uart_data->output_buffer, &write_buf, buf_size);
    if (to_write > 0) {
...
int serial_write(device const *const uart, uint8_t const *data, size_t size,
                 UartData *uart_data) {
...
int serial_read(device const *const uart, uint8_t *data, size_t size,
                UartData *uart_data) {
