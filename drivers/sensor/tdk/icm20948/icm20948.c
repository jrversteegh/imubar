...
static int icm20948_bank_select(const struct device *dev, uint8_t bank) {
  ...
  if (bank != current_bank) {
    ...
    int ret = i2c_reg_update_byte_dt(&config->i2c, REG_BANK_SEL, reg_bank_mask,
                                     bank << 4);
    if (ret) {
...
static inline void icm20948_temp_c(int32_t in, int32_t *out_c,
                                   uint32_t *out_uc) {
...
static inline void icm20948_accel_mss(int64_t sensitivity, int32_t in,
                                      int32_t *out, int32_t *u_out) {
...
static void icm20948_convert_accel(const struct icm20948_config *config,
  ...
  icm20948_accel_mss(sensitivity, raw_accel_value, &output_value->val1,
                     &output_value->val2);
}
...
static int icm20948_convert_gyro(struct sensor_value *val, int16_t raw_val,
  ...
  /* microrad/s */
  val->val2 = (int32_t)((in10_rads - (val->val1 * gyro_sensitivity_10x * 180LL *
                                      1000000LL)) /
...
static inline void icm20948_mag_gauss(int32_t in, int32_t *out,
                                      int32_t *u_out) {
...
static int icm20948_sample_fetch(const struct device *dev,
  ...
  int err = i2c_burst_read_dt(&config->i2c, ACCEL_XOUT_H, read_buff,
                              SENS_READ_BUFF_LEN);
  if (err) {
...
static int icm20948_gyro_config(const struct device *dev) {
  ...
  err = i2c_reg_update_byte_dt(&config->i2c, GYRO_CONFIG_1, GENMASK(5, 0),
                               GYRO_DLPFCFG_0 | config->gyro_fs << 1 |
                                   GYRO_F_ENABLE);
  if (err) {
    return err;
  }
...
static int icm20948_accel_config(const struct device *dev) {
  ...
  err = i2c_reg_update_byte_dt(&config->i2c, ACCEL_CONFIG, GENMASK(5, 0),
                               ACCEL_DLPFCFG_0 | (config->accel_fs << 1) |
...
static int icm20948_mst_rw(const struct device *dev, uint8_t reg, bool write) {
  ...
  ret = i2c_reg_write_byte_dt(&config->i2c, I2C_SLV4_ADDR,
  ...
  /* Initiate transfer  */
  ret = i2c_reg_write_byte_dt(&config->i2c, I2C_SLV4_CTRL,
...
static int icm20948_i2c_master_enable(const struct device *dev, bool enable) {
  ...
  err = i2c_reg_update_byte_dt(&config->i2c, USER_CTRL, BIT(5),
                               enable << 5 | BIT(1));
  if (err) {
  ...
  int ret =
      i2c_reg_update_byte_dt(&config->i2c, INT_PIN_CFG, BIT(1), (!enable) << 1);
  if (ret) {
    return ret;
  }
...
static int icm20948_mag_config(const struct device *dev) {
  ...
  err =
      i2c_reg_update_byte_dt(&config->i2c, I2C_MST_CTRL, BIT(7) | GENMASK(4, 0),
  ...
  // Setting ODR Config
  err = i2c_reg_update_byte_dt(&config->i2c, I2C_MST_ODR_CONFIG, GENMASK(3, 0),
                               0x03);
  if (err) {
  ...
  err = i2c_reg_write_byte_dt(&config->i2c, I2C_SLV0_ADDR,
