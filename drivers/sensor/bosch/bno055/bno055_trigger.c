		    ...
		    const struct sensor_value *val)
{
	...
	if (attr == SENSOR_ATTR_SLOPE_TH) {
		...
		if (i2c_reg_write_byte_dt(&config->i2c,
					  BNO055_REG_SLOPE_TH, (uint8_t)slope_th)
					  < 0) {
		...
		if (i2c_reg_update_byte_dt(&config->i2c,
					   ...
					   val->val1 << BNO055_SLOPE_DUR_SHIFT)
					   < 0) {
...
{
	struct bno055_data *drv_data =
		CONTAINER_OF(cb, struct bno055_data, gpio_cb);
...
static void bno055_thread_cb(const struct device *dev)
{
	...
	err = i2c_reg_read_byte_dt(&config->i2c,
				   BNO055_REG_INT_STATUS_1, &status);
	...
	err = i2c_reg_read_byte_dt(&config->i2c,
				   BNO055_REG_INT_STATUS_0, &status);
...
{
	struct bno055_data *drv_data =
		CONTAINER_OF(work, struct bno055_data, work);
...
int bno055_init_interrupt(const struct device *dev)
{
	...
	k_thread_create(&drv_data->thread, drv_data->thread_stack,
			...
			bno055_thread, drv_data,
			NULL, NULL, K_PRIO_COOP(CONFIG_BNO055_THREAD_PRIORITY),
			0, K_NO_WAIT);
#elif defined(CONFIG_BNO055_TRIGGER_GLOBAL_THREAD)
