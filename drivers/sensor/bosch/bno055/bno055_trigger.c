...
int bno055_attr_set(const struct device *dev, enum sensor_channel chan,
  ...
  if (attr == SENSOR_ATTR_SLOPE_TH) {
    ...
    if (i2c_reg_write_byte_dt(&config->i2c, BNO055_REG_SLOPE_TH,
                              (uint8_t)slope_th) < 0) {
      LOG_DBG("Could not set slope threshold");
      return -EIO;
    }
  } else if (attr == SENSOR_ATTR_SLOPE_DUR) {
    if (i2c_reg_update_byte_dt(&config->i2c, BNO055_REG_INT_5,
...
static void bno055_gpio_callback(const struct device *dev,
...
static void bno055_thread_cb(const struct device *dev) {
  ...
  if (status & BNO055_BIT_DATA_INT_STATUS &&
...
int bno055_trigger_set(const struct device *dev,
  ...
  if (trig->type == SENSOR_TRIG_DATA_READY) {
    ...
    if (i2c_reg_update_byte_dt(&config->i2c, BNO055_REG_INT_EN_1,
                               BNO055_BIT_DATA_EN, 0) < 0) {
    ...
    if (i2c_reg_update_byte_dt(&config->i2c, BNO055_REG_INT_EN_1,
                               BNO055_BIT_DATA_EN, BNO055_BIT_DATA_EN) < 0) {
      LOG_DBG("Could not enable data ready interrupt");
      return -EIO;
    }
  } else if (trig->type == SENSOR_TRIG_DELTA) {
    /* disable any-motion interrupt while changing trigger params */
    if (i2c_reg_update_byte_dt(&config->i2c, BNO055_REG_INT_EN_0,
                               BNO055_SLOPE_EN_XYZ, 0) < 0) {
      LOG_DBG("Could not disable data ready interrupt");
      return -EIO;
    }
    ...
    if (i2c_reg_update_byte_dt(&config->i2c, BNO055_REG_INT_EN_0,
                               BNO055_SLOPE_EN_XYZ, BNO055_SLOPE_EN_XYZ) < 0) {
      LOG_DBG("Could not enable data ready interrupt");
      return -EIO;
    }
  } else {
...
int bno055_init_interrupt(const struct device *dev) {
  ...
  if (i2c_reg_write_byte_dt(&config->i2c, BNO055_REG_INT_RST_LATCH,
                            BNO055_BIT_INT_LATCH_RESET |
                                BNO055_INT_MODE_LATCH) < 0) {
  ...
  gpio_init_callback(&drv_data->gpio_cb, bno055_gpio_callback,
  ...
  if (i2c_reg_update_byte_dt(&config->i2c, BNO055_REG_INT_MAP_1,
  ...
  if (i2c_reg_update_byte_dt(&config->i2c, BNO055_REG_INT_MAP_0,
  ...
  k_thread_create(&drv_data->thread, drv_data->thread_stack,
                  CONFIG_BNO055_THREAD_STACK_SIZE, bno055_thread, drv_data,
                  NULL, NULL, K_PRIO_COOP(CONFIG_BNO055_THREAD_PRIORITY), 0,
                  K_NO_WAIT);
#elif defined(CONFIG_BNO055_TRIGGER_GLOBAL_THREAD)
