...
static int read_i2c_word(const struct device* dev, uint8_t const reg_addr, uint16_t* buf) {
...
static int write_i2c_byte(const struct device* dev, uint8_t const reg_addr, uint8_t value) {
  const struct bmp085_config* config = dev->config;
...
static struct sensor_value get_sensor_value(int32_t const value, int32_t const divisor) {
...
static int bmp085_sample_fetch(const struct device* dev, const enum sensor_channel chan) {
  ...
  switch (chan) {
    case SENSOR_CHAN_PRESS:
      ...
      switch (data->fetch_state) {
        ...
        case bmp085_temperature_fetched:
          if ((ret = write_i2c_byte(
                   dev, BMP085_CONTROL, BMP085_READPRESSURECMD + (BMP085_OVERSAMPLING << 6))) <
              0)
            return ret;
          return ++data->fetch_state;
    ...
    case SENSOR_CHAN_ALL:
      ...
      if ((ret = write_i2c_byte(
               dev, BMP085_CONTROL, BMP085_READPRESSURECMD + (BMP085_OVERSAMPLING << 6))) < 0)
        return ret;
      ...
      data->pressure = (p_buf[0] << 16) + (p_buf[1] << 8) + p_buf[2];
      data->pressure >>= (8 - BMP085_OVERSAMPLING);
      LOG_DBG("Raw temp: %d  Raw pressure: %d", (int)data->temperature, (int)data->pressure);
      return 0;
    default:
      return -ENOTSUP;
  }
...
#define BMP085_DEFINE(inst)                                                                    \
  static struct bmp085_data bmp085_data_##inst;                                                \
                                                                                               \
  static const struct bmp085_config bmp085_config##inst = {.i2c = I2C_DT_SPEC_INST_GET(inst)}; \
                                                                                               \
