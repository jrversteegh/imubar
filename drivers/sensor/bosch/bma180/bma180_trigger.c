...
int bma280_attr_set(const struct device *dev, enum sensor_channel chan,
  ...
  if (attr == SENSOR_ATTR_SLOPE_TH) {
    ...
    if (i2c_reg_write_byte_dt(&config->i2c, BMA280_REG_SLOPE_TH,
                              (uint8_t)slope_th) < 0) {
      LOG_DBG("Could not set slope threshold");
      return -EIO;
    }
  } else if (attr == SENSOR_ATTR_SLOPE_DUR) {
    if (i2c_reg_update_byte_dt(&config->i2c, BMA280_REG_INT_5,
                               BMA280_SLOPE_DUR_MASK,
                               val->val1 << BMA280_SLOPE_DUR_SHIFT) < 0) {
...
static void bma280_gpio_callback(const struct device *dev,
  ...
  struct bma280_data *drv_data = CONTAINER_OF(cb, struct bma280_data, gpio_cb);
...
static void bma280_thread_cb(const struct device *dev) {
  ...
  err = i2c_reg_read_byte_dt(&config->i2c, BMA280_REG_INT_STATUS_1, &status);
  if (status & BMA280_BIT_DATA_INT_STATUS &&
      drv_data->data_ready_handler != NULL && err == 0) {
    drv_data->data_ready_handler(dev, drv_data->data_ready_trigger);
  }

  /* check for any motion */
  err = i2c_reg_read_byte_dt(&config->i2c, BMA280_REG_INT_STATUS_0, &status);
  if (status & BMA280_BIT_SLOPE_INT_STATUS &&
      drv_data->any_motion_handler != NULL && err == 0) {
    drv_data->any_motion_handler(dev, drv_data->any_motion_trigger);

    /* clear latched interrupt */
    err = i2c_reg_update_byte_dt(&config->i2c, BMA280_REG_INT_RST_LATCH,
...
static void bma280_work_cb(struct k_work *work) {
  struct bma280_data *drv_data = CONTAINER_OF(work, struct bma280_data, work);
...
int bma280_trigger_set(const struct device *dev,
  ...
  if (trig->type == SENSOR_TRIG_DATA_READY) {
    ...
    if (i2c_reg_update_byte_dt(&config->i2c, BMA280_REG_INT_EN_1,
    ...
    if (i2c_reg_update_byte_dt(&config->i2c, BMA280_REG_INT_EN_0,
                               BMA280_SLOPE_EN_XYZ, BMA280_SLOPE_EN_XYZ) < 0) {
      LOG_DBG("Could not enable data ready interrupt");
      return -EIO;
    }
  } else {
...
int bma280_init_interrupt(const struct device *dev) {
  ...
  if (i2c_reg_write_byte_dt(&config->i2c, BMA280_REG_INT_RST_LATCH,
                            BMA280_BIT_INT_LATCH_RESET |
  ...
  gpio_init_callback(&drv_data->gpio_cb, bma280_gpio_callback,
  ...
  if (i2c_reg_update_byte_dt(&config->i2c, BMA280_REG_INT_MAP_1,
  ...
  if (i2c_reg_update_byte_dt(&config->i2c, BMA280_REG_INT_MAP_0,
  ...
  k_thread_create(&drv_data->thread, drv_data->thread_stack,
                  CONFIG_BMA280_THREAD_STACK_SIZE, bma280_thread, drv_data,
                  NULL, NULL, K_PRIO_COOP(CONFIG_BMA280_THREAD_PRIORITY), 0,
                  K_NO_WAIT);
#elif defined(CONFIG_BMA280_TRIGGER_GLOBAL_THREAD)
