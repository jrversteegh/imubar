...
static int bma180_sample_fetch(const struct device *dev,
  ...
  case SENSOR_CHAN_ACCEL_XYZ:
    if (i2c_burst_read_dt(&config->i2c, BMA180_REG_ACCEL_X_LSB,
  ...
  case SENSOR_CHAN_DIE_TEMP:
    if (i2c_reg_read_byte_dt(&config->i2c, BMA180_REG_TEMP,
                             (uint8_t *)&data->temp) < 0) {
...
static int bma180_channel_get(const struct device *dev,
                              enum sensor_channel chan,
...
int bma180_init(const struct device *dev) {
  ...
  if (i2c_reg_write_byte_dt(&config->i2c, BMA180_REG_BW,
                            (BMA180_BW << BMA180_BW_SHIFT) |
                                (BMA180_TCS << BMA180_TCS_SHIFT)) < 0) {
    LOG_DBG("Could not set data filter bandwidth");
    return -EIO;
  }

  if (i2c_reg_write_byte_dt(&config->i2c, BMA180_REG_RANGE,
                            BMA180_RANGE << BMA180_RANGE_SHIFT) < 0) {
...
#define BMA180_DEFINE(inst)                                                    \
  ...
  SENSOR_DEVICE_DT_INST_DEFINE(                                                \
      ...
      POST_KERNEL, CONFIG_SENSOR_INIT_PRIORITY, &bma180_driver_api);
