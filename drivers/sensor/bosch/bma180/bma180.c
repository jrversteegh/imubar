...
static int bma280_sample_fetch(const struct device *dev,
  ...
  if (i2c_reg_read_byte_dt(&config->i2c, BMA280_REG_TEMP,
...
static void bma280_channel_accel_convert(struct sensor_value *val,
   ...
   */
  raw_val =
      (raw_val * BMA280_PMU_FULL_RANGE) / (1 << (8 + BMA280_ACCEL_LSB_BITS));
...
int bma280_init(const struct device *dev) {
  ...
  if (i2c_reg_write_byte_dt(&config->i2c, BMA280_REG_PMU_BW, BMA280_PMU_BW) <
      0) {
  ...
  /* set g-range */
  if (i2c_reg_write_byte_dt(&config->i2c, BMA280_REG_PMU_RANGE,
                            BMA280_PMU_RANGE) < 0) {
...
#define BMA280_DEFINE(inst)                                                    \
  ...
  SENSOR_DEVICE_DT_INST_DEFINE(                                                \
      inst, bma280_init, NULL, &bma280_data_##inst, &bma280_config##inst,      \
      POST_KERNEL, CONFIG_SENSOR_INIT_PRIORITY, &bma280_driver_api);
