...
static int h3lis331dl_sample_fetch(const struct device* dev, enum sensor_channel chan) {
  ...
  switch (chan) {
    ...
    case SENSOR_CHAN_ACCEL_XYZ:
      if (i2c_burst_read_dt(
              &config->i2c,
              H3LIS331DL_REG_ACCEL_X_LSB | 0x80,
              (uint8_t*)&data->sample,
...
static int h3lis331dl_channel_get(
...
int h3lis331dl_init(const struct device* dev) {
  ...
  if (i2c_reg_write_byte_dt(&config->i2c, H3LIS331DL_REG_CTRL1, ctrl_reg1) < 0) {
  ...
  if (i2c_reg_write_byte_dt(&config->i2c, H3LIS331DL_REG_CTRL4, ctrl_reg4) < 0) {
