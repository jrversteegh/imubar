...
static inline int l3gd20h_set_odr_raw(const struct device *dev, uint8_t odr) {
  ...
  return i2c_reg_update_byte_dt(&config->i2c, L3GD20H_REG_CTRL_REG1,
...
static const struct {
  int freq_int;
  int freq_micro;
} l3gd20h_odr_map[] = {{12, 500000}, {25, 0},  {50, 0}, {100, 0},
                       {200, 0},     {400, 0}, {800, 0}};

static int l3gd20h_set_odr(const struct device *dev,
...
static inline int l3gd20h_set_fs_raw(const struct device *dev, uint8_t fs) {
  ...
  if (i2c_reg_update_byte_dt(&config->i2c, L3GD20H_REG_CTRL_REG4,
...
static int l3gd20h_sample_fetch(const struct device *dev,
  ...
  case SENSOR_CHAN_GYRO_X:
    return read_i2c(dev, L3GD20H_REG_OUT_X_L, (uint8_t *)&data->sample.x,
                    sizeof(data->sample.x));
  case SENSOR_CHAN_GYRO_Y:
    return read_i2c(dev, L3GD20H_REG_OUT_Y_L, (uint8_t *)&data->sample.y,
                    sizeof(data->sample.y));
  case SENSOR_CHAN_GYRO_Z:
    return read_i2c(dev, L3GD20H_REG_OUT_Z_L, (uint8_t *)&data->sample.z,
                    sizeof(data->sample.z));
  case SENSOR_CHAN_GYRO_XYZ:
    return read_i2c(dev, L3GD20H_REG_OUT_X_L, (uint8_t *)&data->sample,
                    sizeof(data->sample));
...
static inline int l3gd20h_get_channel(enum sensor_channel chan,
                                      struct sensor_value *val,
                                      struct l3gd20h_data *data, int divisor) {
  switch (chan) {
  case SENSOR_CHAN_DIE_TEMP:
    val->val1 = -data->temp;
    val->val2 = 0;
...
static inline int l3gd20h_get_gyro(const struct device *dev,
                                   enum sensor_channel chan,
                                   struct sensor_value *val) {
  struct l3gd20h_data *data = dev->data;
...
static inline int l3gd20h_attr_set_channels(const struct device *dev,
...
static int l3gd20h_attr_set(const struct device *dev, enum sensor_channel chan,
                            enum sensor_attribute attr,
                            const struct sensor_value *val) {
...
static int l3gd20h_init_chip(const struct device *dev) {
  ...
  uint8_t data =
      L3GD20H_DEFAULT_ODR | L3GD20H_DEFAULT_BW | L3GD20H_REG_CTRL_REG1_POWER_UP;
...
#define L3GD20H_DEFINE(inst)                                                   \
  ...
  SENSOR_DEVICE_DT_INST_DEFINE(                                                \
      inst, l3gd20h_init, NULL, &l3gd20h_data_##inst, &l3gd20h_config_##inst,  \
      POST_KERNEL, CONFIG_SENSOR_INIT_PRIORITY, &l3gd20h_api_funcs);
