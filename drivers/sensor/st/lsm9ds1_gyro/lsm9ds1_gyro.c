...
static inline int lsm9ds0_gyro_power_ctrl(const struct device *dev, int power,
  ...
  return i2c_reg_update_byte_dt(
      &config->i2c, LSM9DS0_GYRO_REG_CTRL_REG1_G,
      LSM9DS0_GYRO_MASK_CTRL_REG1_G_PD | LSM9DS0_GYRO_MASK_CTRL_REG1_G_XEN |
          LSM9DS0_GYRO_MASK_CTRL_REG1_G_YEN | LSM9DS0_GYRO_MASK_CTRL_REG1_G_ZEN,
      state);
}
...
static const struct {
  int freq;
  uint8_t reg_val;
} lsm9ds0_gyro_samp_freq_table[] = {{95, 0}, {190, 1}, {380, 2}, {760, 3}};
  ...
  for (i = 0; i < ARRAY_SIZE(lsm9ds0_gyro_samp_freq_table); ++i) {
    if (odr <= lsm9ds0_gyro_samp_freq_table[i].freq) {
      return lsm9ds0_gyro_set_odr_raw(dev,
                                      lsm9ds0_gyro_samp_freq_table[i].reg_val);
    }
  }
...
static int lsm9ds0_gyro_sample_fetch(const struct device *dev,
  ...
  if (i2c_reg_read_byte_dt(&config->i2c, LSM9DS0_GYRO_REG_OUT_X_L_G, &x_l) <
          0 ||
      i2c_reg_read_byte_dt(&config->i2c, LSM9DS0_GYRO_REG_OUT_X_H_G, &x_h) <
          0 ||
      i2c_reg_read_byte_dt(&config->i2c, LSM9DS0_GYRO_REG_OUT_Y_L_G, &y_l) <
          0 ||
      i2c_reg_read_byte_dt(&config->i2c, LSM9DS0_GYRO_REG_OUT_Y_H_G, &y_h) <
          0 ||
      i2c_reg_read_byte_dt(&config->i2c, LSM9DS0_GYRO_REG_OUT_Z_L_G, &z_l) <
          0 ||
      i2c_reg_read_byte_dt(&config->i2c, LSM9DS0_GYRO_REG_OUT_Z_H_G, &z_h) <
          0) {
...
static int lsm9ds0_gyro_init_chip(const struct device *dev) {
  ...
  if (i2c_reg_update_byte_dt(
          &config->i2c, LSM9DS0_GYRO_REG_CTRL_REG4_G,
          LSM9DS0_GYRO_MASK_CTRL_REG4_G_BDU | LSM9DS0_GYRO_MASK_CTRL_REG4_G_BLE,
...
#define LSM9DS0_GYRO_DEFINE(inst)                                              \
  ...
  SENSOR_DEVICE_DT_INST_DEFINE(                                                \
      inst, lsm9ds0_gyro_init, NULL, &lsm9ds0_gyro_data_##inst,                \
      &lsm9ds0_gyro_config_##inst, POST_KERNEL, CONFIG_SENSOR_INIT_PRIORITY,   \
