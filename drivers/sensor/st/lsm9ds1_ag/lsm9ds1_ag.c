...
static inline int lsm9ds1_set_gyro_odr_raw(const struct device* dev, uint8_t odr) {
  const struct lsm9ds1_config* config = dev->config;
...
static const struct {
  int freq_int;
  int freq_micros;
} lsm9ds1_gyro_odr_table[] = {
    {14, 900000}, {59, 500000}, {119, 0}, {238, 0}, {476, 0}, {952, 0}};

static int lsm9ds1_set_gyro_odr(const struct device* dev, const struct sensor_value* odr) {
  uint8_t i;
...
static inline int lsm9ds1_set_accel_odr_raw(const struct device* dev, uint8_t odr) {
  const struct lsm9ds1_config* config = dev->config;
...
static inline int
read_i2c_byte(const struct device* dev, uint8_t const reg_addr, uint8_t* buf) {
  const struct lsm9ds1_config* config = dev->config;
...
static inline int
write_i2c_byte(const struct device* dev, uint8_t const reg_addr, uint8_t const data) {
  const struct lsm9ds1_config* config = dev->config;
...
static int lsm9ds1_sample_fetch(const struct device* dev, enum sensor_channel chan) {
  ...
  switch (chan) {
    case SENSOR_CHAN_ALL:
      return read_i2c(
          dev, LSM9DS1_REG_OUT_X_L_G, (uint8_t*)&data->sample, sizeof(data->sample));
    case SENSOR_CHAN_DIE_TEMP:
      return read_i2c(dev, LSM9DS1_REG_OUT_TEMP_L, (uint8_t*)&data->temp, sizeof(data->temp));
    case SENSOR_CHAN_GYRO_X:
      return read_i2c(
          dev,
          LSM9DS1_REG_OUT_X_L_G,
          (uint8_t*)&data->sample.gyro.x,
          sizeof(data->sample.gyro.x));
    case SENSOR_CHAN_GYRO_Y:
      return read_i2c(
          dev,
          LSM9DS1_REG_OUT_Y_L_G,
          (uint8_t*)&data->sample.gyro.y,
          sizeof(data->sample.gyro.y));
    case SENSOR_CHAN_GYRO_Z:
      return read_i2c(
          dev,
          LSM9DS1_REG_OUT_Z_L_G,
          (uint8_t*)&data->sample.gyro.z,
          sizeof(data->sample.gyro.z));
    case SENSOR_CHAN_GYRO_XYZ:
      return read_i2c(
          dev, LSM9DS1_REG_OUT_X_L_G, (uint8_t*)&data->sample.gyro, sizeof(data->sample.gyro));

    case SENSOR_CHAN_ACCEL_X:
      return read_i2c(
          dev,
          LSM9DS1_REG_OUT_X_L_XL,
          (uint8_t*)&data->sample.accel.x,
          sizeof(data->sample.accel.x));
    case SENSOR_CHAN_ACCEL_Y:
      return read_i2c(
          dev,
          LSM9DS1_REG_OUT_Y_L_XL,
          (uint8_t*)&data->sample.accel.y,
          sizeof(data->sample.accel.y));
    case SENSOR_CHAN_ACCEL_Z:
      return read_i2c(
          dev,
          LSM9DS1_REG_OUT_Z_L_XL,
...
static int lsm9ds1_channel_get(
  ...
  switch (chan) {
    ...
    case SENSOR_CHAN_ACCEL_XYZ:
      *val = get_sensor_value(data->sample.accel.x, get_accel_divisor(data));
      *(val + 1) = get_sensor_value(data->sample.accel.y, get_accel_divisor(data));
      *(val + 2) = get_sensor_value(data->sample.accel.z, get_accel_divisor(data));
      break;
    default:
      return -ENOTSUP;
  }
...
static int lsm9ds1_init_chip(const struct device* dev) {
  ...
  if (write_i2c_byte(
          dev,
          LSM9DS1_REG_CTRL_REG1,
          LSM9DS1_DEFAULT_ODR_G | LSM9DS1_DEFAULT_FS_G | LSM9DS1_DEFAULT_BW_G) < 0) {
  ...
  if (write_i2c_byte(
          dev, LSM9DS1_REG_CTRL_REG3, LSM9DS1_DEFAULT_HPF_ENABLE | LSM9DS1_DEFAULT_HPF_CUTOFF) <
      0) {
  ...
  if (write_i2c_byte(
          dev,
          LSM9DS1_REG_CTRL_REG6,
          LSM9DS1_DEFAULT_ODR_XL | LSM9DS1_DEFAULT_FS_XL | LSM9DS1_DEFAULT_BW_FILTER_ENABLE |
  ...
  dev->data.accel_fs = LSM9DS1_DEFAULT_FS_XL >> LSM9DS1_REG_CTRL_REG6_FS_XL_SHIFT;
#endif
